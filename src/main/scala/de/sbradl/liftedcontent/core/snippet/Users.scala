...
class ManageRoles(user: User) {
  ...
  def addOrRemoveRole(roleId: Long, addOrRemove: Boolean): JsCmd = {
    if (addOrRemove) {
      val userRole = de.sbradl.liftedcontent.core.model.UserRoles.create
      userRole.user(user.id.is)
      userRole.role(roleId)
      userRole.save
    } else {
      val userRole = de.sbradl.liftedcontent.core.model.UserRoles.find(
        By(de.sbradl.liftedcontent.core.model.UserRoles.user, user.id.is),
        By(de.sbradl.liftedcontent.core.model.UserRoles.role, roleId)).open_!
  ...
  def render = {
    ...
    "data-name=title" #> title &
      "data-name=roles *" #> (roles map {
        role =>
          {
            val belongsToRole = de.sbradl.liftedcontent.core.model.UserRoles.count(
              By(de.sbradl.liftedcontent.core.model.UserRoles.user, user.id.is),
              By(de.sbradl.liftedcontent.core.model.UserRoles.role, role.id.is)) != 0
